...
class SwiftContainer: SwiftStyledCollectionElement {
    ...
    private func findParentColumn() -> SwiftColumn? {
    ...
    private func findParentColumnSet(of element: SwiftBaseCardElement) -> SwiftColumnSet? {
    ...
    func deserialize(context: SwiftParseContext, value: [String: Any]) throws -> any SwiftAdaptiveCardElementProtocol {
        ...
        if let itemsArray = value["items"] as? [[String: Any]] {
            ...
            container.items = try itemsArray.map { itemJson in
                ...
                let element = try SwiftBaseCardElement.deserialize(from: mutableJson)
                if let styledElement = element as? SwiftStyledCollectionElement {
