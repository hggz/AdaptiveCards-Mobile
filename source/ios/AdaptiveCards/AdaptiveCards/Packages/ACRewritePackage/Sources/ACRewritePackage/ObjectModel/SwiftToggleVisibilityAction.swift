...
class SwiftToggleVisibilityAction: SwiftBaseActionElement {
    ...
    static func deserialize(from json: [String: Any], context: SwiftParseContext) throws -> SwiftToggleVisibilityAction {
        let toggleVisibilityAction = SwiftToggleVisibilityAction()
        if let targetsArray = json["targetElements"] as? [[String: Any]] {
            toggleVisibilityAction.targetElements = try targetsArray.map {
                ...
                try SwiftToggleVisibilityTarget.deserialize(from: $0)
            }
        } else {
...
class ToggleVisibilityActionParser: SwiftActionElementParser {
    func deserialize(context: SwiftParseContext, from json: [String : Any]) throws -> any SwiftAdaptiveCardElementProtocol {
        return try SwiftToggleVisibilityAction.deserialize(from: json, context: context)
    }
    
    func deserialize(fromString jsonString: String, context: SwiftParseContext) throws -> any SwiftAdaptiveCardElementProtocol {
        return try SwiftToggleVisibilityAction.deserialize(from: jsonString, context: context)
    }
}
