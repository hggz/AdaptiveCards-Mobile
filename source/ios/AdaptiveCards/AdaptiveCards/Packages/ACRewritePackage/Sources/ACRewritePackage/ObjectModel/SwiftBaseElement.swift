...
class SwiftBaseElement: Codable {
    ...
    var id: String?
    var internalId: SwiftInternalId
    var additionalProperties: [String: AnyCodable]?
    var requires: [String: SwiftSemanticVersion]?
    ...
    required init(from decoder: Decoder) throws {
        ...
        self.internalId = try container.decodeIfPresent(SwiftInternalId.self, forKey: .internalId) ?? SwiftInternalId.current()
        self.requires = try container.decodeIfPresent([String: SwiftSemanticVersion].self, forKey: .requires)
        self.fallbackType = try container.decodeIfPresent(SwiftFallbackType.self, forKey: .fallbackType)
        self.fallbackContent = try container.decodeIfPresent(SwiftBaseElement.self, forKey: .fallbackContent)
        ...
        if container.contains(.fallback) {
            if let fallbackString = try? container.decode(String.self, forKey: .fallback) {
                ...
                self.fallbackContent = try SwiftBaseCardElement.deserialize(from: fallbackDict)
            }
        }
