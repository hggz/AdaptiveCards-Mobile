...
protocol MarkDownParsedResultProtocol {
    func translate()
    func addBlockTags()
    func generateHtmlString() -> String
    func appendParseResult(_ result: MarkDownParsedResult)
...
class MarkDownParsedResult: MarkDownParsedResultProtocol {
    ...
    func addBlockTags() {
        if let firstToken = codeGenTokens.first as? MarkDownHtmlGenerator {
            var tokenFirst = firstToken
            tokenFirst.makeItHead()
        }
        if let last = codeGenTokens.last as? MarkDownHtmlGenerator {
            var lastToken = last
    ...
    func addNewLineTokenToParsedResult(_ ch: Character) {
        let htmlToken = MarkDownNewLineHtmlGenerator(token: String(ch))
        appendToTokens(htmlToken)
    }
    ...
    private func markTags(_ token: MarkDownHtmlGenerator) {
        ...
        while let emphasis = currentEmphasis.next() {
            if emphasis.isLeftEmphasis() || (emphasis.isLeftAndRightEmphasis() && leftEmphasisToExplore.isEmpty) {
                ...
                if !currentLeftEmphasis.isMatch(emphasis) {
                    var store: [MarkDownEmphasisHtmlGenerator] = []
                    var isFound = false
