...
final class AdditionalPropertyTests: XCTestCase {
    ...
    func testCanGetAdditionalPropertiesAtCardLevel() throws {
        ...
        XCTAssertEqual(try ParseUtil.jsonToString(value), "{\"unknown\":\"testing unknown\"}\n")
    }
    ...
    func testCanGetAdditionalPropertiesForAction() throws {
        ...
        XCTAssertEqual(try ParseUtil.jsonToString(value), "{\"MyAdditionalProperty\":\"Foo\"}\n")
    }
    ...
    func testCanGetAdditionalPropertiesForInline() throws {
        ...
        let jsonValue = ParseUtil.getJsonValue(from: testJsonString)
        let textRun = try TextRun.deserialize(from: jsonValue)
        let value = textRun?.additionalProperties ?? [:]
        XCTAssertEqual(try ParseUtil.jsonToString(value), "{\"MyAdditionalProperty\":\"Bar\"}\n")
    }
